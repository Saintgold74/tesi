...
class GDOSimulator:
    ...
    def plot_results(self, df_h1, df_h2, df_h3):
        ...
        months = df_h3_int['month'].values
        ax4.bar(months - 0.2, df_h3_sil.groupby('month')['compliance_cost'].mean(), 
                0.4, label='Siloed', alpha=0.7)
        ax4.bar(months + 0.2, df_h3_int.groupby('month')['compliance_cost'].mean(), 
                0.4, label='Integrated', alpha=0.7)
        ax4.set_xlabel('Mesi')
