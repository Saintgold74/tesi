...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{figura 2-1}
...
medi di FV = 127ms (deviazione standard $\sigma= 43ms$), durante i quali i
...
rilevamento dell'85\%. La facilità di rilevazione ha portato a una
...
\subsection{\texorpdfstring{Modellazione della Propagazione negli Ambienti Distribuiti}{2.3.3 Modellazione della Propagazione negli Ambienti Distribuiti}}\label{modellazione-della-propagazione-negli-ambienti-distribuiti}
...
l'adattamento del modello epidemiologico \textbf{SIR
...
=-\beta*S*\frac{I}{N} - \gamma * I \\
\frac{\mathrm{d}R}{\mathrm{d} t}
=\gamma * I
\end{gather}

dove $\beta$ rappresenta il tasso di trasmissione, $\gamma$ il tasso di recovery, S
...
empirica su 15 incidenti reali mostra valori di $\frac{\beta}{\gamma}$ compresi tra 2.3 e
...
Per rendere operativa la metrica ASSA (Aggregated System Surface Attack), l'infrastruttura IT della GDO viene rappresentata come un grafo. I nodi del grafo rappresentano gli asset (server, POS, dispositivi IoT), mentre gli archi rappresentano le connessioni di rete. L'ASSA score viene calcolato come una funzione ponderata di:  
    ...
    \begin{itemize}
        \item Numero di nodi esposti a reti non fidate.
        \item 
        \item Numero di porte e servizi aperti su ciascun nodo.
        \item 
        \item Presenza di vulnerabilità note (CVE) non corrette.
        \item 
        \item Numero di connessioni non crittografate o non autenticate.
    \end{itemize}
...
\begin{gather}
max \ \ \Sigma(w_i \times S_i)\\ \\
soggetto \ \ a: \nonumber\\
\Sigma(C_i) \leqslant Budget \nonumber\\ 
\Sigma(T_i) \leqslant Timeline \nonumber\\ 
O_i \leqslant OpThreshold \ \ \forall i \nonumber\\
\end{gather}

dove $S_i$ rappresenta il miglioramento di sicurezza della misura i, $w_i$ il
peso relativo, $C_i$ il costo, Ti il tempo di implementazione, e $O_i$
l'overhead operativo.

\emph{Nota: I valori presentati di seguito rappresentano proiezioni basate
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{figure/figura 2-5.pdf}
