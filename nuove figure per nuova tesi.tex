...
\usepgfplotslibrary{polar}   % <— aggiungi subito dopo i \usepackage di PGFPlots
\usepackage{xstring}
...
\usetikzlibrary{graphs,er,folding,calendar,matrix}
...
\begin{tikzpicture}[node distance=2cm]
    ...
    \node[physical] (phys) at (0,0) {\textbf{Physical}\\18\%};
    \node[architectural,right=of phys] (arch) {\textbf{Architectural}\\32\%};
    \node[security,right=of arch] (sec) {\textbf{Security}\\28\%};
    \node[compliance,right=of sec] (comp) {\textbf{Compliance}\\22\%};
    ...
    \node[draw,rectangle,left=2cm of decision,fill=blue!20] (balanced) {
        $\displaystyle\sum_{i}(w_i \times C_i)$\\[0.3cm]
        \textit{Balanced}
    };
    
    % Percorso Critical (destra)
    \node[draw,rectangle,right=2cm of decision,fill=red!20] (critical) {
        $\displaystyle\prod_{i}(C_i^{w_i})$\\[0.3cm]
        \textit{Critical}
    };
    
    % Livello 3: Output
    \node[draw,rectangle,above=5cm of decision,minimum width=4cm,minimum height=2cm,fill=green!20] (output) {
        \textbf{GIST Score}\\
        0-100
    };
    
    % Moltiplicatori laterali
    \node[draw,circle,left=of output,fill=yellow!20] (kgdo) {$K_{GDO}$\\1.0-1.5};
    \node[draw,circle,right=of output,fill=orange!20] (innov) {$(1+I)$\\1.0-1.35};
    ...
    % Etichette
    \node[font=\footnotesize,below=0.1cm of phys] {Infrastruttura\\Fisica};
    \node[font=\footnotesize,below=0.1cm of arch] {Maturità\\Architetturale};
    \node[font=\footnotesize,below=0.1cm of sec] {Postura\\Sicurezza};
    \node[font=\footnotesize,below=0.1cm of comp] {Integrazione\\Compliance};
...
\begin{tikzpicture}[scale=0.9]
    ...
    \foreach \comp/\angle/\score in {physical/90/68,arch/0/82,security/270/74,compliance/180/79} {
        ...
        \draw[thick,gistgreen,line width=2pt] ([shift=(\angle:\quadradius)]0,0) ++(0.2,0.8) -- ++(\score/100,0);
    }
    ...
    \foreach \angle/\label/\status in {
        ...
        \def\mcolor{%
      \IfStrEq{\status}{done}{gistgreen}{%
      \IfStrEq{\status}{current}{gistyellow}{gray}}%
    }%
    \node[circle,fill=\mcolor,minimum size=0.3cm] at (\angle:7) {};
    \node[font=\tiny,inner sep=1pt,rotate=\angle,anchor=south] at (\angle:7.5) {\label};
    }
